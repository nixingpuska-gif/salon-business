-- KPI rollup (daily)
-- Aggregates previous full day of activity into kpi_rollup.

with params as (
  select
    (current_date - 1)::date as period_start,
    current_date::date as period_end,
    (current_date - 1)::timestamptz as range_start,
    current_date::timestamptz as range_end
),
tenants_in_scope as (
...
select
  concat(t.tenant_id, ':day:', p.period_start) as id,
  t.tenant_id,
  'day' as granularity,
  p.period_start,
  p.period_end,
