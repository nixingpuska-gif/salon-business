...
let cachedFileTypeFromBuffer:
  | ((buffer: Uint8Array) => Promise<FileTypeResult>)
  | null = null;
...
const getFileTypeFromBuffer = async (
  ...
  if (!cachedFileTypeFromBuffer) {
    ...
    cachedFileTypeFromBuffer = module.fileTypeFromBuffer as (
      data: Uint8Array,
    ) => Promise<FileTypeResult>;
  }
...
const uploadToCloudflare = async (
  ...
  const detectedType = await getFileTypeFromBuffer(fs.readFileSync(filePath));
