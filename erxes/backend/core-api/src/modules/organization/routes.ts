...
router.get('/get-frontend-plugins', async (_req: Request, res: Response) => {
  ...
  const ENABLED_PLUGINS_UI = getEnv({
    name: 'ENABLED_PLUGINS_UI',
    defaultValue: '',
  });
  const VERSION = getEnv({ name: 'VERSION', defaultValue: 'os' });
  const PLUGINS_BASE_URL = getEnv({
    name: 'PLUGINS_BASE_URL',
    defaultValue: 'https://plugins.erxes.io/latest',
  });
  ...
  if (VERSION === 'saas') {
    ...
    Object.keys(charges).forEach((key) => {
      if (
        ...
        if (enabledPluginsArray.includes(pluginName)) {
          remotes.push({
            name: `${pluginName}_ui`,
            entry: `${PLUGINS_BASE_URL}/${pluginName}_ui/remoteEntry.js`,
          });
        }
      }
    });
    ...
    if (pluginsSource) {
      pluginsSource.split(',').forEach((plugin) => {
        remotes.push({
          name: `${plugin}_ui`,
          entry: `${PLUGINS_BASE_URL}/${plugin}_ui/remoteEntry.js`,
        });
      });
    }
